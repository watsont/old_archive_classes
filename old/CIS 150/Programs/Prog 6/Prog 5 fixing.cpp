...
void SubjAvg(int stuMemLoc, int stuCount, float SubjectAverage[],
              float Scores[][SCORE_COLMS]);
int SortName (ostream& outData, string LstName[], string FstName[],
              int stuID[], float Scores[][SCORE_COLMS], float StudentAverage[],
              string FullArray[][FULL_ARRAY_LEN], int stuCount);
     

int main()
{
	ifstream inData;                                                            //Used to obtain data from file 
	ofstream outData;                                                            //Used to store values from arrays; then store info in new array
	...
	string FullArray[MAX_STUDENTS][FULL_ARRAY_LEN];                               //Array to hold ALL information
    ...
    */
    //Prints out information sorted out
    cout << "\n***Data sorted alphabetically by student's last name***\n";
    outData.open("sorted_progData.txt");
    SortName (outData, LstName, FstName, stuID, Scores,
             StudentAverage, FullArray, stuCount);
    outData.close();
    system("PAUSE");
	return 0;
}
//******************************************************************************
int SortName (ostream& outData, string LstName[], string FstName[],             //This function serves to create a new file containing
              int stuID[], float Scores[][SCORE_COLMS], float StudentAverage[], //known info and then organizes the data according to
              string FullArray[][FULL_ARRAY_LEN], int stuCount)                 //alphabetical order (last name)
{
    if (!outData) {                                                                     //If no file found, exit
        ...
        for (int i = 0; i < stuCount; i++) {                                            //Loop stores data from outData into array FullArray.
            ...
            for (int j = 0; j < SCORE_COLMS + 4; j++) {                                 //Since "cin >>  " will put data in until it reaches
                ...
                outData >> FullArray[i];                                                //store for (num of subject columns + 4; twice for Last
